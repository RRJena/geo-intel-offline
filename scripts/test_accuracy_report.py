...
def test_country(
    ...
    if point_in_polygon_with_holes(centroid, exterior_tuples, holes_tuples):
    ...
    while len(test_points) < num_test_points and attempts < max_attempts * 2:
...
def generate_accuracy_report(
    ...
    report_lines.append(f"Generated: {Path(output_file).stat().st_mtime if Path(output_file).exists() else 'N/A'}\n")
    ...
    for continent in sorted(continent_results.keys()):
        result = continent_results[continent]
        report_lines.append(
            f"| {continent} | {result['total_countries']} | {result['tested_countries']} | "
            f"{result['total_tests']} | {result['passed']} | {result['failed']} | "
    ...
    sorted_results = sorted(country_results, key=lambda x: x['accuracy'], reverse=True)
    
    for rank, result in enumerate(sorted_results, 1):
        report_lines.append(
            f"| {rank} | {result['name']} | {result['iso2'] or 'N/A'} | {result['iso3'] or 'N/A'} | "
    ...
    report_lines.append(f"- **Overall Accuracy**: {overall_accuracy:.2f}%\n")
    report_lines.append(f"- **Countries with 100% accuracy**: {accuracy_ranges['Perfect (100%)']}\n")
    report_lines.append(f"- **Countries with 90%+ accuracy**: {accuracy_ranges['Perfect (100%)'] + accuracy_ranges['Excellent (90-99%)']}\n")
    report_lines.append(f"- **Countries needing improvement**: {len(low_accuracy)}\n\n")
    
    report_lines.append("### Test Methodology\n\n")
    report_lines.append(f"- Test points per country: {num_test_points_per_country}\n")
    report_lines.append("- Points are sampled from within each country's polygon\n")
